...
async def cmd_search(message: Message):
    ...
    logger.info(f"🔍 /search uid={uid} q={query!r}")
...
async def cmd_help(message: Message):
    ...
    admin_members = (
        ...
        "\n/add_admin &lt;id&gt; — Назначить администратором"
        "\n/remove_user &lt;id&gt; — Удалить участника"
...
async def inline_search(query: InlineQuery):
    ...
    results_data = await api_client.search(q)
    if not results_data:
        await query.answer(
            results=[InlineQueryResultArticle(
                id="noresult",
    ...
    await query.answer(results=items, cache_time=30)  # type: ignore[arg-type]
