...
_redis: Any = None
_redis_disabled_until: float = 0
_redis_lock = asyncio.Lock()


async def get_redis():
    """Возвращает Redis клиент или None если REDIS_URL не задан или лимит превышен."""
    global _redis, _redis_disabled_until
    
    if _redis_disabled_until > time.time():
        return None
    ...
    async with _redis_lock:
        ...
        try:
            ...
            if "limit exceeded" in msg.lower():
                logger.error("Redis LIMIT EXCEEDED. Disabling cache for 5 min.")
                _redis_disabled_until = time.time() + 300
            else:
