...
const TagResultsView = lazy(() => import('@/views/TagResultsView').then(m => ({ default: m.TagResultsView })))
  ...
  return (
    <Suspense fallback={<ViewLoader />}>
      <Routes>
        ...
        />
        <Route path="/category/:id" element={<InnerGuidesView onSelectGuide={handleOpenGuide} cats={cats} />} />
        ...
        <Route path="/favorites" element={<FavoritesView favorites={favorites} onSelectGuide={handleOpenGuide} onToggle={toggleFav} />} />
        <Route path="/history" element={<HistoryView history={history} onSelectGuide={handleOpenGuide} />} />
...
// Route wrappers
const InnerGuidesView = ({ onSelectGuide, cats }: any) => {
  const { id } = useParams()
  const cat = cats?.find((c: any) => c.key === id)
  const category = { key: id!, title: cat?.title || 'Гайды', icon: cat?.icon }
  return <GuidesView category={category} onSelectGuide={onSelectGuide} />
}

const InnerGuideView = ({ onToggleFavorite, isFavorite, onOpenGuide, onTagClick, onGuideLoaded }: any) => {
  const { id } = useParams()
  return (
    <GuideView
      guideKey={id!}
...
const InnerTagResultsView = ({ onSelectGuide }: any) => {
  const { tag } = useParams()
