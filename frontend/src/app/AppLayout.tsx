import { FC, ReactNode, Suspense, lazy, useState, useMemo } from 'react'
  ...
  return (
    <MotionConfig reducedMotion="user">
      <div className="app-shell flex h-[var(--tg-viewport-stable-height,100dvh)] flex-col overflow-hidden bg-background text-foreground">
            ...
            <div className="flex w-full items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="flex size-10 items-center justify-center overflow-hidden rounded-2xl border border-border/10 bg-background shadow-lg shadow-primary/20">
                  {!logoFailed ? (
                    <img src={logoSrc} alt="BlackRose" className="size-full object-cover" onError={() => setLogoFailed(true)} />
                  ) : (
              ...
              <div className="flex items-center gap-2">
                {isAdmin ? (
                  <Button variant="ghost" size="sm" className="h-10 rounded-2xl border border-primary/30 px-3 text-[11px] font-black uppercase tracking-[0.1em] text-primary" onClick={() => push({ type: 'admin' })}>
                    Панель
                  </Button>
                ) : !isTMA ? (
                  <Button variant="ghost" size="sm" className="h-10 rounded-2xl border border-border/40 px-3 text-[11px] font-black uppercase tracking-[0.1em] text-foreground/70" onClick={() => sheet.present({ type: 'login' })}>
                    Вход
                  </Button>
                ) : null}
                <Button
                  variant="ghost" size="icon" className="size-10 rounded-2xl"
                  onClick={() => {
        ...
        <FabButton visible={fabVisible} label="Навигация" onBack={handleBack} onHoldComplete={() => sheet.present({ type: 'quickNav' })} />

        <Toaster position="top-center" toastOptions={{ className: 'glass-card text-foreground rounded-3xl p-4 font-bold shadow-2xl transition-all border-none scale-110', duration: 3000 }} />

        {sheet.item?.type === 'login' && (
          <Suspense fallback={null}>
            <AdminLoginModal onSuccess={() => { setIsAdmin(true); sheet.dismiss(); push({ type: 'admin' }) }} onClose={sheet.dismiss} />
          </Suspense>
        )}

        {sheet.item?.type === 'quickNav' && (
          <Suspense fallback={null}>
            <QuickNav categories={cats || []} onSelect={(cat) => { push({ type: 'category', id: cat.key }); sheet.dismiss() }} onHome={() => { push({ type: 'home' }); sheet.dismiss() }} onClose={sheet.dismiss} />
          </Suspense>
        )}
      </div>
    </MotionConfig>
