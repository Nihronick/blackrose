...
import { Shield, Users, Target, Trophy, Edit2, UserPlus } from 'lucide-react'
...
export const GuildRosterView: FC<GuildRosterViewProps> = ({ guildId }) => {
  ...
  const getRowClass = (status: string) => {
    switch (status) {
      case 'trial': return 'bg-amber-500/10 hover:bg-amber-500/20'
      case 'left': return 'bg-red-500/10 hover:bg-red-500/20 opacity-75'
      case 'reserve': return 'bg-blue-500/10 hover:bg-blue-500/20'
      case 'active':
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      case 'guild_master': return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/20 text-amber-500 border border-amber-500/20">Мастер</span>
      case 'guild_vice_master': return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-300/20 text-slate-300 border border-slate-300/20">Вице-мастер</span>
      default: return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-muted text-muted-foreground border border-border/10">Участник</span>
    }
  }

  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'trial': return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/10 text-amber-500">Испытательный</span>
      case 'left': return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-red-500/10 text-red-500">Покинул</span>
      case 'reserve': return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-blue-500/10 text-blue-500">Резерв</span>
      case 'active':
      default: return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-green-500/10 text-green-500">Актив</span>
    }
  }
      ...
      <div className="p-4 md:p-8">
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-6 mb-8 stagger-in">
          <div className="glass-card rounded-2xl p-4 md:p-5 relative overflow-hidden group">
            ...
            <div className="text-2xl md:text-3xl font-black mt-2 md:mt-4">{stats?.member_count || 0}</div>
            <div className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-muted-foreground mt-1">Участников</div>
          </div>
          <div className="glass-card rounded-2xl p-4 md:p-5 relative overflow-hidden group">
            <div className="absolute top-2 right-2 p-2 bg-amber-500/10 rounded-xl">
              <Trophy className="size-4 md:size-5 text-amber-500" />
            </div>
            <div className="text-2xl md:text-3xl font-black mt-2 md:mt-4 text-amber-500">{stats?.total_ranks || 0}</div>
            <div className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-muted-foreground mt-1">Сумма рангов</div>
          </div>
          <div className="glass-card rounded-2xl p-4 md:p-5 relative overflow-hidden group">
            <div className="absolute top-2 right-2 p-2 bg-blue-500/10 rounded-xl">
              <Target className="size-4 md:size-5 text-blue-500" />
            </div>
            <div className="text-2xl md:text-3xl font-black mt-2 md:mt-4 text-blue-500">{(stats?.average_rank || 0).toFixed(1)}</div>
            <div className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-muted-foreground mt-1">Средний ранг</div>
          </div>
        </div>
        ...
        <div className="glass-card rounded-[24px] overflow-hidden">
          {isLoading ? (
            ...
            <div className="p-6 text-center text-muted-foreground">В гильдии пока нет участников.</div>
          ) : (
              ...
              <table className="w-full text-sm text-left whitespace-nowrap">
                ...
                <tbody className="divide-y divide-border/10">
                  {roster.map((member, index) => (
                    <tr key={member.id} className={`transition-colors ${getRowClass(member.status)}`}>
