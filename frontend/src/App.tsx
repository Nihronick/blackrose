  ...
  <div className="flex h-full flex-col overflow-hidden px-5 pt-8">
    ...
    <div className="grid grid-cols-1 gap-5">
      {[1, 2, 3, 4].map((i) => (
        <div
          key={i}
          className="h-28 rounded-[32px] border border-border/10 skeleton shadow-sm"
        />
      ))}
    </div>
  </div>
...
export const App: React.FC = () => {
  ...
  const { isAdmin, cats, language, setLanguage, setIsAdmin, setCats, theme, setTheme, setIsTMA } =
    useAppStore()
  ...
  const headerTitle = useMemo(() => {
    ...
    // Для гайдов заголовок обычно управляется внутри компонента через document.title, 
    // но здесь мы можем вернуть название раздела
  ...
  // FAB should only be visible on listing pages (Categories, Tags) 
  // On Guide pages, we use the Header back button (Web) or Native back button (Telegram)
  const fabVisible = isCategory || isTag
  const fabLabel = 'Навигация'
  const logoSrc = `${import.meta.env.BASE_URL}app-icon.png`

  return (
    <MotionConfig reducedMotion="user">
      <div
        className="app-shell flex h-[var(--tg-viewport-stable-height,100dvh)] flex-col overflow-hidden bg-background text-foreground"
